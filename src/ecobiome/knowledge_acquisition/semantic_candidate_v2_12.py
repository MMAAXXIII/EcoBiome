"""Provider-neutral canonical semantic candidates — EcoBiome V2.12.

V2.12 sits after V2.9 Claim/Evidence admission and deterministic grounding.
...
from ecobiome.knowledge_acquisition.semantic_grounding import (
    audit_arguments_v1_3 as audit_arguments,
)
...
SEMANTIC_CANDIDATE_V2_12_SCHEMA_VERSION = (
    "ecobiome-canonical-semantic-candidate-v2.12"
)
SEMANTIC_CANDIDATE_V2_12_CONTRACT_NAME = (
    "ecobiome-canonical-semantic-candidate"
)
SEMANTIC_CANDIDATE_V2_12_CONTRACT_VERSION = "2.12"
RELATION_TYPE_BASIS_VERSION = "2.12"
...
class SemanticCandidateV212Error(ValueError):
    """Raised when a V2.12 semantic candidate violates the frozen contract."""


def _require_mapping(value: object, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise SemanticCandidateV212Error(f"{label} must be an object")
    return value


def _require_nonempty_string(value: object, label: str) -> str:
    if not isinstance(value, str) or not value:
        raise SemanticCandidateV212Error(f"{label} must be a non-empty string")
    return unicodedata.normalize("NFC", value)


def _reject_native_floats(value: object, path: str = "candidate") -> None:
    if isinstance(value, float):
        raise SemanticCandidateV212Error(
            f"native float forbidden in canonical V2.12 payload: {path}"
        )
...
def _normalized_source_surface(source_text: str, value: str) -> str:
    ...
    if not stripped:
        raise SemanticCandidateV212Error("opaque source text must be non-empty")
...
def _canonical_argument_value(
    *,
    ...
    if role in OPAQUE_OPEN_TEXT_ROLES_V1_3:
        if not isinstance(raw_value, str):
            raise SemanticCandidateV212Error(
    ...
    raise SemanticCandidateV212Error(f"unsupported V2.12 role: {role}")
...
def _candidate_identity_projection(
    ...
    if not isinstance(projected, dict):
        raise SemanticCandidateV212Error("candidate projection must be object")
    return projected


def semantic_candidate_sha256_v2_12(
    candidate: Mapping[str, Any],
) -> str:
...
def _contract_descriptor(registry_v2_10: Mapping[str, Any]) -> dict[str, str]:
    ...
    descriptor: dict[str, str] = {
        "name": SEMANTIC_CANDIDATE_V2_12_CONTRACT_NAME,
        "version": SEMANTIC_CANDIDATE_V2_12_CONTRACT_VERSION,
...
def build_semantic_candidate_v2_12(
    survivor: Mapping[str, Any],
    source_request: Mapping[str, Any],
    registry_v2_10: Mapping[str, Any],
) -> dict[str, Any]:
    """Build one provider-neutral V2.12 candidate from an admitted survivor."""
    ...
    if spec.get("semantic_type_contract_state") == "unresolved_blocked":
        raise SemanticCandidateV212Error(
            f"fail-closed relation cannot enter V2.12: {relation}"
        )
    ...
    candidate: dict[str, Any] = {
        "schema_version": SEMANTIC_CANDIDATE_V2_12_SCHEMA_VERSION,
    ...
    candidate["canonical_candidate_sha256"] = (
        semantic_candidate_sha256_v2_12(candidate)
    )
...
def build_semantic_candidates_v2_12(
    ...
    for raw_survivor in survivors:
        ...
        candidate = build_semantic_candidate_v2_12(
            survivor,
            source_request,
            registry_v2_10,
        )
    ...
    return {
        "schema_version": "ecobiome-semantic-candidate-batch-v2.12",
...
def validate_semantic_candidate_v2_12(
    candidate: Mapping[str, Any],
) -> Mapping[str, Any]:
    """Validate a complete V2.12 candidate and its provider-neutral identity."""
    expected_root = {
        "schema_version",
        "contract",
        "source",
    ...
    if set(candidate) != expected_root:
        raise SemanticCandidateV212Error(
            "unexpected V2.12 candidate root keys"
        )
    if candidate.get("schema_version") != SEMANTIC_CANDIDATE_V2_12_SCHEMA_VERSION:
        raise SemanticCandidateV212Error(
            "unexpected V2.12 schema_version"
        )
    if candidate.get("automatic_scientific_acceptance") is not False:
        raise SemanticCandidateV212Error(
            "V2.12 must not grant automatic scientific acceptance"
        )
    if candidate.get("promotion_readiness") not in {
        ...
        raise SemanticCandidateV212Error(
            "unsupported V2.12 promotion_readiness"
        )
    ...
    expected_hash = semantic_candidate_sha256_v2_12(candidate)
    ...
    if set(contract) != expected_contract_keys:
        raise SemanticCandidateV212Error(
            "unexpected V2.12 contract descriptor keys"
        )
    if contract.get("name") != SEMANTIC_CANDIDATE_V2_12_CONTRACT_NAME:
        raise SemanticCandidateV212Error(
            "unexpected V2.12 contract name"
        )
    if contract.get("version") != SEMANTIC_CANDIDATE_V2_12_CONTRACT_VERSION:
        raise SemanticCandidateV212Error(
            "unexpected V2.12 contract version"
        )
    ...
    if set(source) != {
        ...
        raise SemanticCandidateV212Error(
            "unexpected V2.12 source descriptor keys"
        )
    ...
    if set(semantic) != {
        ...
        raise SemanticCandidateV212Error(
            "unexpected V2.12 semantic descriptor keys"
        )
    ...
    for raw_argument in arguments:
        ...
        if set(argument) != {
            ...
            raise SemanticCandidateV212Error(
                "unexpected V2.12 argument keys"
            )
        ...
        if kind not in {
            ...
            raise SemanticCandidateV212Error(
                f"unsupported V2.12 value kind: {kind!r}"
            )
        if kind == "decimal":
...
def canonical_semantic_candidate_json_v2_12(
    candidate: Mapping[str, Any],
) -> str:
    """Return canonical UTF-8 JSON text for a validated V2.12 candidate."""
    validate_semantic_candidate_v2_12(candidate)
    return canonical_json_text(candidate)


def render_semantic_candidate_review_text_v2_12(
    candidate: Mapping[str, Any],
) -> str:
