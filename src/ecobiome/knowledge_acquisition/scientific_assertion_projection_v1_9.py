"""Fail-closed Projection V1.9 for V2.12 candidates and Entity Resolution V1.1."""
...
from ecobiome.knowledge_acquisition.semantic_candidate_entity_resolution_v1_1 import (
    ENTITY_RESOLUTION_POLICY_SHA256 as ENTITY_RESOLUTION_POLICY_V1_1_SHA256,
)
from ecobiome.knowledge_acquisition.semantic_candidate_review_v1_1 import (
    SemanticCandidateReviewV11Error,
    require_candidate_acceptance_v1_1,
)
...
PROJECTION_CONTRACT_VERSION = "1.9"
PROJECTION_CONTRACT_SHA256 = (
    "c7802b7d63f60ca52b8de233be8d6a0c39e46779a0bf7211fcbf1a3f69fb1280"
)
ENTITY_RESOLUTION_POLICY_SHA256 = ENTITY_RESOLUTION_POLICY_V1_1_SHA256
...
_PROJECTION_SPECS = (
    ...
    ProjectionSpecV1(
        spec_id="nitrogen_oxidized_from_to.nitrogen_oxidation.process.v1",
        semantic_type="nitrogen_oxidation",
        relation="nitrogen_oxidized_from_to",
        assertion_kind="process",
        predicate="nitrogen_oxidized_from_to",
        builder="spec_ternary_entity_relation_v1",
        role_classes=(
            ("source_material", ENTITY_ARGUMENT),
            ("target_material", ENTITY_ARGUMENT),
            ("process_agent", ENTITY_ARGUMENT),
        ),
    ),
    ProjectionSpecV1(
        spec_id="nitrogen_assimilated_from_into.nitrogen_assimilation.process.v1",
        semantic_type="nitrogen_assimilation",
        relation="nitrogen_assimilated_from_into",
        assertion_kind="process",
        predicate="nitrogen_assimilated_from_into",
        builder="spec_ternary_entity_relation_v1",
        role_classes=(
            ("source_material", ENTITY_ARGUMENT),
            ("target_nitrogen_pool", ENTITY_ARGUMENT),
            ("process_agent", ENTITY_ARGUMENT),
        ),
    ),
)
...
if canonical_sha256(PROJECTION_CONTRACT_DESCRIPTOR_V1_9) != PROJECTION_CONTRACT_SHA256:
    raise RuntimeError("Scientific Assertion Projection V1.9 identity mismatch")
...
def candidate_argument_sha256_v1(argument: Mapping[str, Any]) -> str:
    """Bind a human-reviewed mapping to one exact V2.12 argument."""
...
def _build_spec_ternary_entity_relation(
    *,
    spec: ProjectionSpecV1,
    candidate: Mapping[str, Any],
    arguments: Mapping[str, Mapping[str, Any]],
    ...
    if spec.builder != "spec_ternary_entity_relation_v1":
        raise ScientificAssertionProjectionV1Error(
            f"unsupported ternary entity projection spec: {spec.spec_id}"
        )
    if context_resolutions:
        raise ScientificAssertionProjectionV1Error(
            "ternary entity relation accepts no context reconstruction"
        )
    if len(spec.role_classes) != 3 or any(
        role_class != ENTITY_ARGUMENT for _, role_class in spec.role_classes
    ):
        raise ScientificAssertionProjectionV1Error(
            "ternary entity relation requires exactly three ENTITY_ARGUMENT roles"
        )
    participants: list[dict[str, object]] = []
    for role, _ in spec.role_classes:
        entity = _validate_entity_resolution(
            role, arguments[role], entity_resolutions.get(role)
        )
        participants.append({"role": role, "entity": entity})
    semantic = _candidate_semantic(candidate)
    payload = canonical_assertion_payload(
    ...
    canonical_participants = payload.get("participants")
    if not isinstance(canonical_participants, list) or len(canonical_participants) != 3:
        raise ScientificAssertionProjectionV1Error(
            "canonical ternary relation must contain exactly three participants"
        )
    normalized_parts: list[str] = []
    for participant in canonical_participants:
        if not isinstance(participant, Mapping):
        ...
        role = _require_nonempty_string(
            participant.get("role"), "canonical participant role"
        )
        participant_entity = participant.get("entity")
        if not isinstance(participant_entity, Mapping):
            raise ScientificAssertionProjectionV1Error(
        ...
        normalized_parts.append(f'{role}=entity_ref("{entity_id}",{revision})')
    return payload, f"{spec.predicate}(" + ", ".join(normalized_parts) + ")"


def project_scientific_assertion_v1_9(
    candidate: Mapping[str, Any],
    *,
    ...
    segments: Mapping[str, SegmentsRow],
    segment_reviews: Mapping[str, Sequence[SegmentReviewEventsRow]],
    entity_resolution_policy_sha256: str,
    entity_resolutions: Mapping[str, ReviewedEntityArgumentV1],
    context_resolutions: Mapping[str, ReviewedContextArgumentV1] | None = None,
) -> dict[str, Any]:
    """Project one reviewed V2.12 candidate or fail closed without persistence."""
    if entity_resolution_policy_sha256 != ENTITY_RESOLUTION_POLICY_SHA256:
        raise ScientificAssertionProjectionV1Error(
            "entity-resolution policy identity mismatch"
        )
    validate_semantic_candidate_v2_12(candidate)
    try:
        candidate_review_trace = require_candidate_acceptance_v1_1(
            candidate,
            candidate_reviews,
        )
    except SemanticCandidateReviewV11Error as exc:
    ...
    elif spec.builder == "spec_ternary_entity_relation_v1":
        assertion_payload, normalized_text = _build_spec_ternary_entity_relation(
            spec=spec,
            candidate=candidate,
            arguments=arguments,
            entity_resolutions=entity_resolutions,
