"""Provider-error-tolerant structured semantic benchmark core for EcoBiome.

Semantic Evaluator V2.3.2. Candidate/provider output defects are scored and
reported instead of crashing the benchmark. Fixture, semantic-export, and
evaluator-contract corruption remain fatal. Blind alignment does not require
Golden facet labels.
"""
...
def _pair_quality(
    ...
    comparison = _compare_meaning(expected, candidate["benchmark_meaning"])
    status_rank = {
        "match": 4,
        "ambiguous": 3,
        "overclaimed": 2,
...
def _align_group(
    ...
    """Deterministic one-to-one alignment without using Golden facet labels."""
    ...
    while remaining_gold and remaining_candidates:
        ...
        for gold_id, gold in remaining_gold.items():
            for candidate_index, candidate in remaining_candidates:
                quality = _pair_quality(gold, candidate)
...
def evaluate_structured_semantic_benchmark(
    ...
    *,
    ...
    for group, gold_items in required_groups.items():
        ...
        assignments, leftovers = _align_group(gold_items, candidates)
        ...
        for gold in gold_items:
            ...
            elif comparison["status"] in {"ambiguous", "overclaimed"}:
    ...
    return {
        "schema_version": "2.3.2-provider-errors-scoreable-prototype",
