...
export function StatusBadge({ status }: { status: WaterBodyStatus }) {
  if (status === 'unknown') {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full bg-slate-700/40 px-2 py-1 text-xs text-slate-300">
...
export function StatusDot({ status }: { status: WaterBodyStatus }) {
  ...
  const glow =
    status === 'stable'
      ? 'shadow-[0_0_8px_rgba(52,211,164,0.6)]'
      : status === 'warning'
